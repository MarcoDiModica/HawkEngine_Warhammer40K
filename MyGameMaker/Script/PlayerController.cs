...
//        else
//        {
//            //Set the collider mass (dont remove this, otherwise collider could not be properly initialized)
//            rb.SetMass(1.0f);
//        }
//    }
...
public class PlayerController : MonoBehaviour
{
    ...
    public override void Start()
    {
        ...
        playerDash = new PlayerDash(gameObject);
        playerShooting = new PlayerShooting(gameObject);
    }

    public override void Update(float deltaTime)
    {
        Vector3 moveDirection = playerInput.GetInputDirection();

        if (!playerDash.IsDashing)
        {
            playerMovement.UpdateMovement(moveDirection, deltaTime);
            playerMovement.UpdateRotation(moveDirection, deltaTime);
            ...
            playerShooting.UpdateShooting(deltaTime);
        }
        else
        {
            playerDash.HandleActiveDash(deltaTime);
        }

        playerDash.UpdateDashRecharge(deltaTime);
        playerShooting.UpdateProjectiles(deltaTime);
        playerShooting.CleanupProjectiles();
    }
}
