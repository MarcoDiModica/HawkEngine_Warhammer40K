...
void main() {
    ...
    // Clamp roughness to prevent visual artifacts
    ...
    // Directional light
    {
        ...
        // Energy conservation: reflected light (kS) + refracted light (kD)
        vec3 kS = F;
        vec3 kD = vec3(1.0) - kS;
    ...
    for(int i = 0; i < numPointLights && i < MAX_POINT_LIGHTS; i++) {
        ...
        float falloffStart = pointLights[i].radius * 0.8;
        attenuation *= smoothstep(pointLights[i].radius, falloffStart, distance);
        ...
        // Scale by incident light angle
        float NdotL = max(dot(N, L), 0.0);
        
        // Final point light contribution
        Lo += (kD * albedo.rgb / PI + specular) * radiance * NdotL;
        
        // Add ambient contribution from each light (already attenuated)
        Lo += pointLights[i].ambient * albedo.rgb * ao * attenuation * 0.1;
    }
    
    // Global ambient light 
    vec3 ambient = dirLight.ambient * albedo.rgb * ao;
    
    // Final color combines ambient, emissive, and direct lighting
    vec3 color = ambient + emission + Lo;
    
    // Tone mapping (prevents bright areas from being clipped)
    color = color / (color + vec3(1.0));
    
    // Gamma correction (ensures proper brightness on monitors)
