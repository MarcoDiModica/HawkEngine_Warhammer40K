...
size_t GenerateNewMeshID(ModelData& model) {
...
void Mesh::loadToOpenGL()
{
	...
	if (model->GetModelData().vertexData.size() > 0 && model->GetModelData().vertexData[0].m_BoneIDs[0] != -1) {
		...
		glVertexAttribIPointer(5, 4, GL_INT, sizeof(glm::ivec4), (const void*)0);
		...
		glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(glm::vec4), (const void*)0);
	...
	if (model->GetID() == 0) {
		model->SetID(GenerateNewMeshID(model->GetModelData()));
	}
