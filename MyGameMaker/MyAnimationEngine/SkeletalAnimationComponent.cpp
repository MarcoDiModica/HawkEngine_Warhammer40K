...
void SkeletalAnimationComponent::SaveBinary(const std::string& filename) const
{
	...
	if (!fout.is_open()) {
		LOG(LogType::LOG_ERROR, "Error al guardar la animaci�n esqueletal: %s", fullPath.c_str());
		return;
	}
	...
	LOG(LogType::LOG_INFO, "Animaci�n esqueletal guardada correctamente: %s", fullPath.c_str());
}
...
bool SkeletalAnimationComponent::LoadBinary(const std::string& filename)
{
	...
	if (!fin.is_open()) {
		LOG(LogType::LOG_ERROR, "Error al cargar la animaci�n esqueletal: %s", fullPath.c_str());
		return false;
	}
	...
	LOG(LogType::LOG_INFO, "Animaci�n esqueletal cargada correctamente: %s", fullPath.c_str());
	return true;
}
