		...
		if (gameObject->HasComponent<LightComponent>()) {
			...
			if (light->GetLightType() == LightType::DIRECTIONAL) {
				...
				if (pointLights.size() < MAX_POINT_LIGHTS) {
					...
					LOG(LogType::LOG_WARNING, "Warning: Excedido m�ximo de luces puntuales");
				}
			}
		}
	}
...
void ForwardPlusLighting::PerformLightCulling(const glm::mat4& viewMatrix, const glm::mat4& projMatrix) {
	...
	// Barrera de memoria para asegurar que los resultados est�n disponibles
	glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);

	glUseProgram(0);

	// Por ahora, asumimos que todas las luces son visibles para estad�sticas
...
void ForwardPlusLighting::CPUFallbackCulling(const glm::mat4& viewMatrix, const glm::mat4& projMatrix) {
	...
	for (int tileIdx = 0; tileIdx < totalTiles; tileIdx++) {
		...
		// Asignar �ndices de luces al tile
...
bool ForwardPlusLighting::CompileLightCullingShader() {
	const char* lightCullingSource = R"(
        ...
        struct PointLight {
            vec4 position;    // xyz = posici�n, w = radio
            vec4 color;       // rgb = color, a = intensidad
            vec4 attenuation; // x = constante, y = lineal, z = cuadr�tica, w = no usado
            uint lightType;   // 0 = point, 1 = spot (futuro)
            uint castShadow;  // 0 = no, 1 = yes
            uint shadowMapIndex; // �ndice a shadowmap (si usa sombras)
            uint padding;     // alineaci�n
        };
        ...
        // Buffer para �ndices de luces por tile
        ...
        // Verifica si la esfera est� delante del plano
        ...
        void main() {
            ...
            // Funci�n para crear plano a partir de 3 puntos
            // Esta funci�n asume que los puntos est�n en sentido horario vistos desde el lado positivo del plano
            vec3 a, b, c, normal;
            float d;
            ...
            // Recorrer todas las luces y verificar intersecci�n con frustum
            for (uint i = lightIndexStart; i < numLights; i += lightIndexStep) {
                ...
                // Si es visible, a�adir a la lista compartida
                if (isVisible) {
