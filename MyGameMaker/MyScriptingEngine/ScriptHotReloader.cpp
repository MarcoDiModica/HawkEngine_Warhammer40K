...
bool ScriptHotReloader::CompileExistingProject() {
	...
	if (resultFile.is_open()) {
		resultFile << "C�digo de salida: " << exitCode;
		resultFile.close();
	}
	...
	if (prefer) {
		if (m_MSBuildPath.empty() || !std::filesystem::exists(m_MSBuildPath)) {
			if (!FindWorkingMSBuild()) {
				LOG(LogType::LOG_WARNING, "Preferencia establecida a MSBuild, pero no se encontr� una instalaci�n. Se usar� dotnet si est� disponible.");
			}
		}
	}
	else {
		if (m_DotnetPath.empty() || !std::filesystem::exists(m_DotnetPath)) {
			if (!FindWorkingDotnet()) {
				LOG(LogType::LOG_WARNING, "Preferencia establecida a dotnet, pero no se encontr� una instalaci�n. Se usar� MSBuild si est� disponible.");
			}
		}
	}
}
...
void ScriptHotReloader::SaveBuildPreference(bool preferMSBuild) {
	try {
		...
		LOG(LogType::LOG_WARNING, "Error al guardar la preferencia de compilaci�n");
	}
}
...
bool ScriptHotReloader::LoadBuildPreference() {
	try {
		...
		LOG(LogType::LOG_WARNING, "Error al cargar la preferencia de compilaci�n");
	}
